...
export default class InventoryFirebase {
    ...
    handleFirebaseItemAdded(snapshot, folderName) {
        ...
        if (data._folder) {
            ...
            if (!this.inventory.items[itemName]) {
                ...
                this.inventory.ui.notify(`New item "${itemName}" added from remote`);
                this.inventory.ui.render();
            }
        }
    }
    ...
    handleFirebaseItemRemoved(snapshot, folderName) {
        ...
        if (data && data._folder) {
            ...
            if (this.inventory.items[itemName]) {
                ...
                this.inventory.ui.notify(`Item "${itemName}" removed from remote`);
                this.inventory.ui.render();
            }
        }
    }
    ...
    async checkRemoteStatus() {
        if (this.inventory.currentFolder) {
            ...
            this.inventory.isRemote = folder && folder.remote === true;
        } else {
    ...
    async copyFirebaseRef() {
        ...
        if (!firebasePath) {
            this.inventory.ui.notify('Unable to get Firebase path');
            return;
        }
        
        try {
            await navigator.clipboard.writeText(firebasePath);
            this.inventory.ui.notify('Firebase path copied to clipboard!');
        } catch (error) {
            console.error('Failed to copy to clipboard:', error);
            this.inventory.ui.notify('Failed to copy path');
        }
    }
    ...
    getCurrentFirebasePath() {
        ...
        if (this.inventory.currentFolder) {
            const sanitizedFolder = this.sanitizeFirebasePath(this.inventory.currentFolder);
    ...
    isItemInRemoteLocation() {
        if (this.inventory.currentFolder) {
            ...
            const folderData = this.inventory.folders[this.inventory.currentFolder];
    ...
    async makeRemote() {
        if (!SM.scene?.localUser?.name) {
            this.inventory.ui.notify('User not logged in. Cannot upload to Firebase.');
            return;
        }
        ...
        if (totalSize > oneMB) {
            const sizeMB = (totalSize / oneMB).toFixed(2);
            const confirmed = confirm(`Upload size is ${sizeMB} MB. This may take a while. Continue?`);
            if (!confirmed) {
                return;
            }
        }
        ...
        this.inventory.ui.notify('Uploading to Firebase...');
    ...
    async uploadToFirebase(contents, userName) {
        try {
            ...
            if (!window.firebase || !window.firebase.database) {
                this.inventory.ui.notify('Firebase not initialized. Please check configuration.');
                return;
            }
            ...
            this.inventory.ui.notify('Successfully uploaded to Firebase!');
            ...
            this.inventory.ui.notify(`Upload failed: ${error.message}`);
        }
    }
    ...
    async importFromFirebase(firebaseRef) {
        if (!window.firebase || !window.firebase.database) {
            this.inventory.ui.notify('Firebase not initialized');
            return false;
        }
        ...
        try {
            ...
            if (importedCount > 0) {
                ...
                this.inventory.ui.notify(`Successfully imported ${importedCount} item(s)`);
                ...
                this.inventory.ui.notify('No new items to import');
                return false;
            }
            
        } catch (error) {
            console.error('Firebase import error:', error);
            this.inventory.ui.notify(`Import failed: ${error.message}`);
            return false;
        }
    }
    ...
    async uploadImageToFirebase(file) {
        try {
            // Show upload notification
            this.inventory.ui.notify('Uploading image...');
            
            // Get Firebase Storage from networking module
            if (!window.networking || !window.networking.getStorage) {
                this.inventory.ui.notify('Firebase Storage not initialized. Please wait and try again.');
                return;
            }
            
            const storage = window.networking.getStorage();
            if (!storage) {
                this.inventory.ui.notify('Firebase Storage not available');
                return;
            }
            ...
            this.inventory.ui.notify(errorMessage);
        }
    }
